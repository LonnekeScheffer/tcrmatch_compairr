...
def match_with_compairr(seq_file1, seq_file2, out_folder, compairr_path="/storage/lonnekes/TCRMatch_CompAIRR/compairr/src/compairr"):
    ...
    cmd_args = [str(compairr_path), str(seq_file1), str(seq_file2),
                "-m", "--pairs", str(pairs_path), "--cdr3", "-d", "1", "--indels", "--distance",
...
def process_pairs_file(pairs_file, tmp_folder, max_pairs_per_distance=1000):
    ...
    for chunk in df:
        for index, row in chunk.iterrows():
            if row["distance"] == 1:
                if len(row["cdr3_aa_1"]) != len(row["cdr3_aa_2"]):
...
distances_tcrmatch_df.to_csv(str(folder / "distances_tcrmatch_scores_indels.tsv"), sep="\t", index=False)
