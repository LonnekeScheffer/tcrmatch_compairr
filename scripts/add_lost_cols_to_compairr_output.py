...
def add_cols(iedb_file, pairs_file, output_file):
    iedb = pd.read_csv(iedb_file, sep="\t", usecols=['sequence_id', 'repertoire_id', 'junction_aa', 'original_seq', 'receptor_group', 'epitopes', 'source_organisms', 'source_antigens'])
    iedb.rename(columns={"junction_aa": "trimmed_seq"}, inplace=True)
